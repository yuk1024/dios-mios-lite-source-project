...
{
	int i;
	void *ptr = (void*)0x13010000;

#if 1
	ehci->periodic = /*ehci_maligned(DEFAULT_I_TDPS * sizeof(__le32),32,4096)*/ ptr;
	ptr+=DEFAULT_I_TDPS * sizeof(__le32);
	...
	{
            ehci->qtds[i] = /*ehci_maligned(sizeof(struct ehci_qtd),32,4096)*/ptr;
			ptr+=sizeof(struct ehci_qtd);
	}

	ehci->qtd_used = 0;
	ehci->asyncqh = /*ehci_maligned(sizeof(struct ehci_qh),32,4096)*/ptr;
	...
	ehci->async = /*ehci_maligned(sizeof(struct ehci_qh),32,4096)*/ptr;
	ptr+=sizeof(struct ehci_qh);
