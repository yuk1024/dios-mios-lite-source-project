	...
	if( read32(CARD_CONTROL) != 0xdeadbeef )
	{
		...
		switch( read32(CARD_SCMD) >> 24 )
		{
			...
			case 0xC0:
			{
				...
				dbgprintf("MC:CARDOpen( \"%s\", 0x%x )", FileName, FInfo );
			...
			case 0xC1:
			{
				CARDFileInfo *CFile = (CARDFileInfo *)P2C(read32(CARD_SCMD_1));

				dbgprintf("MC:CARDClose( 0x%p, %d )", CFile, CFile->fileNo );
			...
			case 0xC3:
			{
				...
				if( CS.length == 0 )
				{
					...
					memcpy( (void*) P2C(read32(CARD_SCMD_2)), &CS, sizeof(CARDStat) );
