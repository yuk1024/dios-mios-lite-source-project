...
found:
	...
	dev->buffer =/* USB_Alloc(MAX_TRANSFER_SIZE)*/ (void*)0x130A0100;;
...
s32 USBStorage_Inquiry(usbstorage_handle *dev, u8 lun)
{
	...
	u8 *response = /*USB_Alloc(36)*/ (void*)0x1307000;
	dbgprintf("Alloc:%p Line:%d\n", response, __LINE__ );
...
s32 USBStorage_ReadCapacity(usbstorage_handle *dev, u8 lun, u32 *sector_size, u32 *n_sectors)
{
	...
	u8 *response = /*USB_Alloc(8)*/ (void*)0x1307040;
	dbgprintf("Alloc:%p Line:%d\n", response, __LINE__ );
     u32 val;
...
s32 USBStorage_Try_Device(struct ehci_device *fd)
{
	...
	{
		dbgprintf("Could not open USB device\n");
		return -EINVAL;
	}
