...
static int ehci_init(void)
{
	...
	ehci->ctrl_buffer =  /*USB_Alloc(sizeof(usbctrlrequest))*/ (void*)0x130A0020;
	dbgprintf("Alloc:%p",ehci->ctrl_buffer);
...
s32 ehci_control_message(struct ehci_device *dev,u8 bmRequestType,u8 bmRequest,u16 wValue,u16 wIndex,u16 wLength,void *buf)
{
	...
	if( urb.transfer_buffer_length )
	{
		...
		urb.transfer_buffer = (void*)0x0D404000;
		memcpy( urb.transfer_buffer, buf, wLength );
		
		dc_invalidaterange( urb.transfer_buffer, wLength );

		ahb_flush_to(AHB_EHCI);
		ahb_flush_from(AHB_STARLET);

		ret = ehci_do_urb( dev, &urb );
		dbgprintf("ehci_do_urb():%d\n", ret );
		
		ahb_flush_from(AHB_EHCI);
		hexdump( urb.transfer_buffer, wLength ); 
